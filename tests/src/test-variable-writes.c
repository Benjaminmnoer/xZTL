...
/* Object Size */
#define TEST_BUFFER_SZ (4080) /* 12 MB */
...
static void cunit_variablew_assert_ptr (char *fn, void *ptr)
{
...
static void cunit_variablew_assert_int (char *fn, uint64_t status)
{
    CU_ASSERT (status == 0);
    if (status)
	printf ("\n %s: %lx\n", fn, status);
...
static int cunit_variablew_init (void)
{
    return 0;
}

static int cunit_variablew_exit (void)
{
    return 0;
}

static void test_variablew_init (void)
{
    int ret;

    ret = zrocks_init (*devname);
    cunit_variablew_assert_int ("variablew_init", ret);
}

static void test_variablew_exit (void)
{
    zrocks_exit ();
}

static void test_variablew_fill_buffer (uint32_t id)
{
    uint32_t byte;
    uint8_t value = 0x1;
...
static int test_variablew_check_buffer (uint32_t id, uint32_t off, uint32_t size)
{
    /*printf (" \nMem check:\n");
...
static void test_variablew_new (void)
{
    ...
    for (id = 0; id < ids; id++) {
        ...
        test_variablew_fill_buffer (id);

        ret[id] = zrocks_new (id + 1, wbuf[id], size, level);

        printf("Writing object finished for id: %lu. Return code %u\n", id, ret[id]);

        cunit_variablew_assert_int ("zrocks_new", ret[id]);
    }
}
...
static void test_variablew_read (void)
{
	...
	while (offset < size) {
	    ...
	    cunit_variablew_assert_int ("zrocks_read_obj", ret[id]);
	    if (ret[id])
	...
	ret[id] = test_variablew_check_buffer (id, 0, TEST_BUFFER_SZ);
	cunit_variablew_assert_int ("zrocks_read_obj:check", ret[id]);
	if (ret[id])
...
int main (int argc, const char **argv)
{
    ...
    pSuite = CU_add_suite("Suite_zrocks", cunit_variablew_init, cunit_variablew_exit);
    if (pSuite == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }
    ...
    if ((CU_add_test (pSuite, "Initialize ZRocks",
		      test_variablew_init) == NULL) ||
	(CU_add_test (pSuite, "Variable write",
		      test_variablew_new) == NULL) ||
	(CU_add_test (pSuite, "Read object",
		      test_variablew_read) == NULL) ||
    (CU_add_test (pSuite, "Close ZRocks",
              test_variablew_exit) == NULL)) {
	CU_cleanup_registry();
	return CU_get_error();
    }
